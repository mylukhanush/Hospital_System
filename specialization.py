...
class Specialization:
    ...
    def Check_in_patient(self, name, status):
        ...
        if status not in self.PATIENT_STATUS_NUMBERS:
            print("Invalid status. Status should be 0 (normal), 1 (Emergency).")
            return
    ...
    def format_patient_status(status):
        ...
        # return "Super-Urgent"
    @staticmethod
    def Doctors_Avaliable(self):
        Doctors_Aval = ["venu_ortho", "charan_psychology", "bunny_gastrology"]
        print(Doctors_Aval)
